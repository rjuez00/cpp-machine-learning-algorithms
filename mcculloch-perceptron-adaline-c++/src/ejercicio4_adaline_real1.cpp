...
int main(int argc, char** argv) {  
    ...
    if (argc < 5){
        ...
        std::cout << "ejemplo " << argv[0] << "inputfile(string) learn_rate(float) proporcionTrain(float) threshold_adaline\n";
        exit(1);
    }
    

    float learn_rate = atof(argv[2]);
    float proporcionTrain = atof(argv[3]);
    ...
    //EJECUCION
    entrenamiento_adaline(atributosEntrenamiento, clasesEntrenamiento, red, entradaC, salidaC, threshold, learn_rate, 5000, &atributosTest, &clasesTest);
