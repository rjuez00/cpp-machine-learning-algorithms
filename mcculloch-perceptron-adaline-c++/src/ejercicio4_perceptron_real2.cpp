...
int main(int argc, char** argv) {
    ...
    if (argc < 6){
        //./real2 inputfile_train inputfile_test output_file learnrate umbral 
        std::cout << "Haz make ayuda_perceptron para entender el uso de argumentos\n";
        std::cout << "ejemplo " << argv[0] << "inputfile_train(string) inputfile_test(string) output_file(string) learnrate(float) umbral(float) \n";
        exit(1);
    }

    float learn_rate = atof(argv[4]);
    float umbral = atof(argv[5]);
    ...
    //EJECUCION
    entrenamiento_perceptron(atributosEntrenamiento, clasesEntrenamiento, red, entradaC, salidaC, learn_rate, 5000);

    //GENERA PREDICCIONES TEST
    Vec2Df predicciones = predecir_arg_max(red, atributosTest);
    escribirVec2Df(predicciones, argv[3]);
