...
int main(int argc, char** argv) {  
    ...
    if (argc < 4){
        //./logicos inputfile learnrate threshold_adaline
        std::cout << "Haz make ayuda_perceptron para entender el uso de argumentos\n";
        std::cout << "ejemplo " << argv[0] << "inputfile(string) learn_rate(float) threshold_adaline(float)\n";
    }
    ...
    //EJECUCION
    entrenamiento_adaline(atributosEntrenamiento, clasesEntrenamiento, red, entradaC, salidaC, threshold, learn_rate);
